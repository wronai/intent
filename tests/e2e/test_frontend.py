...
API_URL = os.getenv("TEST_API_URL", f"http://localhost:{APP_PORT}")
...
class TestFormSubmission:
    ...
    async def test_intent_form_component(self, page: Page):
        ...
        await expect(form).to_have_class(/loading/)
...
class TestDataComponents:
    ...
    async def test_intent_data_component(self, page: Page):
        ...
        await expect(data_el).not_to_have_class(/loading/)
...
class TestErrorHandling:
    ...
    async def test_api_error_response(self, page: Page):
        ...
        # Mock API error
        await page.route(f"{API_URL}/**", lambda route: route.fulfill(
            status=500,
