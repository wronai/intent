...
def middleware(phase: Union[MiddlewarePhase, str] = MiddlewarePhase.BEFORE, priority: int = 0):
    """
...
class HookRegistry:
    ...
    def on(self, event: Union[HookEvent, str]):
...
def hook(event: Union[HookEvent, str]):
            ...
            for field, rules in schema.get('properties', {}).items():
                if field in data:
                    ...
                    if rules.get('minLength') and len(str(value)) < rules['minLength']:
                        errors.append(f"{field} must be at least {rules['minLength']} characters")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Log audit entry
                logger.info(f"AUDIT: {action} | args={args} | kwargs={kwargs} | "
