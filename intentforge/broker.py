...
class MQTTMessage:
    ...
    def from_bytes(cls, topic: str, data: bytes) -> "MQTTMessage":
        return cls(topic=topic, payload=json.loads(data.decode("utf-8")))
...
class MQTTIntentBroker:
    ...
    def __init__(
        ...
        # Callbacks
        self._on_intent: Callable | None = None
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self._client = mqtt.Client(
                client_id=self.client_id, callback_api_version=mqtt.CallbackAPIVersion.VERSION2
            )
        ...
        try:
            ...
            if self.forge:
                asyncio.run_coroutine_threadsafe(
                    self._process_and_respond(intent, client_id), self._loop
                )
    ...
    def _send_error(self, client_id: str, error: str):
        ...
        response_topic = MQTTTopic.INTENT_RESPONSE.value.format(client_id=client_id)
        self._publish(response_topic, {"success": False, "error": error})
    ...
    def _publish_capabilities(self):
        ...
        capabilities = {
            ...
            "features": ["code_generation", "validation", "caching", "auto_deploy"],
        }
