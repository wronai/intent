    ...
    def _validate_sql(self, code: str, result: ValidationResult) -> None:
        ...
        for pattern, message in required_patterns:
            if not re.search(pattern, code_upper):
                result.add_warning(ValidationError(level=ValidationLevel.SYNTAX, message=message))
    ...
    def _validate_html(self, code: str, result: ValidationResult) -> None:
        ...
        try:
            ...
            for error in validator.errors:
                result.add_error(ValidationError(level=ValidationLevel.SYNTAX, message=error))
            for tag in validator.tag_stack:
                result.add_warning(
                    ValidationError(level=ValidationLevel.SYNTAX, message=f"Unclosed tag: <{tag}>")
                )
...
class SecurityValidator(BaseValidator):
    ...
    DANGEROUS_PATTERNS = {
        ...
        "sql": [
            ...
            (
                r";\s*UPDATE\s+\w+\s+SET.*WHERE\s+1\s*=\s*1",
                SecurityRisk.CRITICAL,
                "Unrestricted UPDATE",
            ),
        ],
            ...
            class SecurityVisitor(ast.NodeVisitor):
                ...
                def visit_Call(self, node):
                    ...
                    if func_name in dangerous_funcs:
                        self.issues.append((func_name, dangerous_funcs[func_name], node.lineno))
                ...
                def visit_Import(self, node):
                    for alias in node.names:
                        if alias.name in ("pickle", "marshal", "ctypes"):
                            self.issues.append(
                                (f"import {alias.name}", SecurityRisk.HIGH, node.lineno)
                            )
                    self.generic_visit(node)
    ...
    def _validate_js_semantics(self, code: str, result: ValidationResult) -> None:
        ...
        for pattern, message in patterns:
            if re.search(pattern, code):
                result.add_warning(ValidationError(level=ValidationLevel.SEMANTIC, message=message))
...
class CodeValidator:
    ...
    def __init__(self, sandbox_mode: bool = True, settings: ValidationSettings | None = None):
    ...
    async def validate(
        ...
        if len(code) > self.settings.max_code_length:
            return False, [
                f"Code exceeds maximum length of {self.settings.max_code_length} characters"
            ]
    ...
    def validate_sync(
        self, code: str, language: str, level: ValidationLevel = ValidationLevel.FULL
    ) -> ValidationResult:
