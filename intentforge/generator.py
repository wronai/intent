...
from .config import get_settings, LLMSettings
...
RETURNING *""",
    ...
    def generate(self, intent: Intent, context: GenerationContext) -> str:
        ...
        if any(word in description for word in ['select', 'get', 'fetch', 'find', 'list', 'pobierz']):
        ...
        elif any(word in description for word in ['update', 'modify', 'change', 'zmień', 'aktualizuj']):
...
"""
    ...
    def _generate_schema(self, intent: Intent, context: GenerationContext) -> str:
        ...
        table = intent.context.get('table', 'items')
        fields = intent.context.get('fields', {
...
class DOMGenerator(DSLGenerator):
    ...
    def _generate_form_handler(self, intent: Intent, context: GenerationContext) -> str:
        ...
        field_extraction = '\n        '.join(
            f'const {f} = form.querySelector(\'[name="{f}"]\')?.value?.trim();'
            for f in fields
        )
        
        validation = '\n        '.join(
            f'if (!{f}) errors.push("{f.capitalize()} is required");'
            for f in fields
        )
