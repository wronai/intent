...
from typing import Optional, Dict, Any, Generic, TypeVar, Tuple
from dataclasses import dataclass, field, asdict
import logging
import threading
from collections import OrderedDict

from .config import get_settings, CacheSettings
...
class IntentCache:
    ...
    def get_result(self, intent: Intent) -> Optional[IntentResult]:
    ...
    def set_result(self, intent: Intent, result: IntentResult, ttl: Optional[int] = None) -> None:
    ...
    def find_similar(self, description: str, intent_type: str, threshold: float = 0.8) -> Optional[IntentResult]:
        """
        Find similar cached result using fuzzy matching
        Useful when exact match isn't found
        """
        partial_fp = self.fingerprint_gen.generate_partial(description, intent_type)
