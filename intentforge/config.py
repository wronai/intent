...
class DatabaseSettings(BaseSettings):
    ...
    def safe_connection_string(self) -> str:
        ...
        return self.connection_string.replace(self.password.get_secret_value(), "****")


class MQTTSettings(BaseSettings):
    """MQTT broker configuration"""

    model_config = SettingsConfigDict(env_prefix="MQTT_", env_file=".env", extra="ignore")
...
class ValidationSettings(BaseSettings):
    ...
    forbidden_imports: list[str] = Field(
        default_factory=lambda: [
            ...
            "__import__",
            "compile",
            "open",  # open allowed only in sandbox
        ]
    )
...
class Settings(BaseSettings):
    ...
    def validate_environment(cls, v: str) -> str:
        env_aliases = {"dev": "development", "prod": "production", "stage": "staging"}
...
class ConfigLoader:
    ...
    def __init__(self, env_files: list[str] | None = None):
